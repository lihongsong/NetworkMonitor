...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NetworkModel *currenModel=[self modelForTableView:tableView atIndexPath:indexPath];
    ...
    requestHTTPMethod = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@   %@   %@",currenModel.requestHTTPMethod,currenModel.responseMIMEType,[((NetworkModel *)((httpRequests)[indexPath.row])).startDateString substringFromIndex:5]]
                                                           attributes:@{
                                                                        NSFontAttributeName : detailFont,
                                                                        NSForegroundColorAttributeName: detailColor
                                                                        }];
...
- (void)updateSearchResultsWithSearchString:(NSString *)searchString {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *tempFilterHTTPRequests = [httpRequests filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(NetworkModel *httpRequest, NSDictionary *bindings) {
...
- (NetworkModel *)modelForTableView:(UITableView *)tableView atIndexPath:(NSIndexPath *)indexPath {
    NetworkModel *currenModel = [[NetworkModel alloc] init];
    if (tableView == mySearchDisplayController.searchResultsTableView) {
        currenModel=(NetworkModel *)((filterHTTPRequests)[indexPath.row]);
    }else{
        currenModel=(NetworkModel *)((httpRequests)[indexPath.row]);
    }
    return currenModel;
}
